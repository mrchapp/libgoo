...
#define ID "OMX.TI.AAC.encode"
#define DASF_PARAM_NAME "OMX.TI.index.config.aacencHeaderInfo"
#define DATAPATH_PARAM_NAME "OMX.TI.index.config.aac.datapath"
#define STREAMID_PARAM_NAME "OMX.TI.index.config.aacencstreamIDinfo"

#define FRAMESPERBUFFER "OMX.TI.index.config.aacencframesPerOutBuf"

#define BITRATEMODE_DEFAULT  0
#define OUTPUTFRAMES_DEFAULT 12

G_DEFINE_TYPE (GooTiAacEnc, goo_ti_aacenc, GOO_TYPE_TI_AUDIO_ENCODER)

enum _GooTiAacEncProp
{
	PROP_0,
	PROP_BITRATEMODE,
	PROP_OUTPUTFRAMES
...
goo_ti_aacenc_bitratemode_get_type ()
{
	...
	if (type == 0)
	{
		const static GEnumValue values[] = {
			{ GOO_TI_AACENC_BR_CBR, "0", "Constant Bit Rate" },
			{ GOO_TI_AACENC_BR_VBR1, "1",
			  "Variable Bit Rate Quality Level 1" },
			{ GOO_TI_AACENC_BR_VBR2, "2",
			  "Variable Bit Rate Quality Level 2" },
			{ GOO_TI_AACENC_BR_VBR3, "3",
			  "Variable Bit Rate Quality Level 3" },
			{ GOO_TI_AACENC_BR_VBR4, "4",
			  "Variable Bit Rate Quality Level 4" },
			{ GOO_TI_AACENC_BR_VBR5, "5",
			  "Variable Bit Rate Quality Level 5" },
			{ 0, NULL, NULL },
		};

		type = g_enum_register_static ("GooTiAacEncBitRateMode", values);
	}
...
static guint
_output_buffer_size (GooTiAacEnc* self)
{
	g_assert (GOO_IS_TI_AACENC (self));
...
_validate_bit_rate (OMX_AUDIO_PARAM_AACPROFILETYPE* param)
{
	...
	return (param->nBitRate >= 8000 && param->nBitRate <= 576000);
}
...
static void
goo_ti_aacenc_set_property (GObject *object, guint prop_id,
			    const GValue* value, GParamSpec *spec)
{
	g_assert (GOO_IS_TI_AACENC (object));
	GooTiAacEnc *self = GOO_TI_AACENC (object);

	switch ( prop_id )
	{
	case PROP_BITRATEMODE:
		_goo_ti_aacenc_set_bit_rate_mode (self,
...
static void
goo_ti_aacenc_get_property (GObject *object, guint prop_id,
...
static void
goo_ti_aacenc_init (GooTiAacEnc* self)
{
...
static void
goo_ti_aacenc_finalize (GObject* object)
{
	g_assert (GOO_IS_TI_AACENC (object));
	GooTiAacEnc* self = GOO_TI_AACENC (object);
...
static void
goo_ti_aacenc_load_parameters (GooComponent* component)
{
	g_assert (GOO_IS_TI_AACENC (component));
	GooTiAacEnc* self = GOO_TI_AACENC (component);
...
static void
goo_ti_aacenc_set_parameters (GooComponent* component)
{
	g_assert (GOO_IS_TI_AACENC (component));
	GooTiAacEnc* self = GOO_TI_AACENC (component);
...
static void
goo_ti_aacenc_validate_ports (GooComponent* component)
{
	g_assert (GOO_IS_TI_AACENC (component));
	GooTiAacEnc* self = GOO_TI_AACENC (component);
	g_assert (self->input_port_param != NULL);
	...
	_goo_ti_aacenc_set_output_frames (self);

	/* input param */
	{
		OMX_AUDIO_PARAM_PCMMODETYPE* param = NULL;
		param = GOO_TI_AACENC_GET_INPUT_PORT_PARAM (self);
	...
	/* input */
	{
		...
		param->nBufferSize = GOO_TI_AACENC_INPUT_BUFFER_SIZE;
	...
	/* output */
	{
		...
		profile = GOO_TI_AACENC_GET_OUTPUT_PORT_PARAM (self);
...
static void
goo_ti_aacenc_class_init (GooTiAacEncClass* klass)
{
	GObjectClass* g_klass = G_OBJECT_CLASS (klass);
	g_klass->finalize = goo_ti_aacenc_finalize;

	g_klass->set_property =	goo_ti_aacenc_set_property;
	g_klass->get_property = goo_ti_aacenc_get_property;
	...
	o_klass->load_parameters_func = goo_ti_aacenc_load_parameters;
	o_klass->set_parameters_func = goo_ti_aacenc_set_parameters;
	o_klass->validate_ports_definition_func = goo_ti_aacenc_validate_ports;
	o_klass->eos_flag_func = goo_ti_aacenc_eos_buffer_flag;
