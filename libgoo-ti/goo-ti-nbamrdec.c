...
#define ID "OMX.TI.AMR.decode"
#define DASF_PARAM_NAME "OMX.TI.index.config.nbamrheaderinfo"
#define DATAPATH_PARAM_NAME "OMX.TI.index.config.nbamr.datapath"
#define ACOUSTIC_PARAM_NAME "OMX.TI.index.config.nbamrheaderinfo"
#define STREAMID_PARAM_NAME "OMX.TI.index.config.nbamrstreamIDinfo"
#define INPUT_NBAMRDEC_BUFFERSIZE 118
#define INPUT_NBAMRDEC_BUFFERSIZE_MIME 34
#define OUTPUT_BUFFERSIZE 320

G_DEFINE_TYPE (GooTiNbAmrDec, goo_ti_nbamrdec, GOO_TYPE_TI_AUDIO_DECODER)

static void
goo_ti_nbamrdec_init (GooTiNbAmrDec* self)
{
...
static void goo_ti_nbamrdec_finalize(GObject* object)
{
    g_assert (GOO_IS_TI_NBAMRDEC(object));
    GooTiNbAmrDec* self = GOO_TI_NBAMRDEC (object);
    ...
    (*G_OBJECT_CLASS (goo_ti_nbamrdec_parent_class)->finalize)(object);

    return;
}

static void goo_ti_nbamrdec_load_parameters(GooComponent* component)
{
    g_assert (GOO_IS_TI_NBAMRDEC(component));
    GooTiNbAmrDec *self = GOO_TI_NBAMRDEC (component);
    g_assert (self->input_port_param == NULL);
    g_assert (self->output_port_param == NULL);
    g_assert (component->cur_state != OMX_StateInvalid);

    GOO_OBJECT_DEBUG (self, "goo_ti_nbamrdec_load_parameters\n");
...
static void goo_ti_nbamrdec_set_parameters(GooComponent* component)
{
    g_assert (GOO_IS_TI_NBAMRDEC(component));
    GooTiNbAmrDec* self = GOO_TI_NBAMRDEC (component);
    g_assert (self->input_port_param != NULL);
    ...
    goo_component_set_parameter_by_index (component, OMX_IndexParamAudioAmr,
            self->input_port_param);
...
static void goo_ti_nbamrdec_validate_ports_definitions(GooComponent* component)
{
    g_assert (GOO_IS_TI_NBAMRDEC(component));
    GooTiNbAmrDec* self = GOO_TI_NBAMRDEC (component);
    g_assert (self->input_port_param != NULL);
    ...
    GOO_OBJECT_DEBUG (self, "goo_ti_nbamrdec_validate_ports_definitions\n");
    ...
    /* input */
    {
        ...
        GOO_PORT_GET_DEFINITION (port)->nBufferSize = INPUT_NBAMRDEC_BUFFERSIZE;
...
static void goo_ti_nbamrdec_class_init(GooTiNbAmrDecClass* klass)
{
    GObjectClass* g_klass = G_OBJECT_CLASS (klass);
    g_klass->finalize = goo_ti_nbamrdec_finalize;

    GooComponentClass* o_klass = GOO_COMPONENT_CLASS (klass);
    o_klass->load_parameters_func = goo_ti_nbamrdec_load_parameters;
    o_klass->set_parameters_func = goo_ti_nbamrdec_set_parameters;
    o_klass->validate_ports_definition_func
            = goo_ti_nbamrdec_validate_ports_definitions;
