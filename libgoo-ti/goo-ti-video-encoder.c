...
#define ID "OMX.TI.Video.encoder"

enum _GooTiVideoEncoderProp
{
        PROP_0,
	PROP_CONTROLRATE,
	PROP_FRAMEINTERVAL
...
#define DEFAULT_CONTROL_RATE GOO_TI_VIDEO_ENCODER_CR_VARIABLE
#define DEFAULT_DEBLOCK_FILTER OMX_TRUE
#define DEFAULT_QPI 1
#define DEFAULT_FRAME_INTERVAL 30
#define DEFAULT_OUTPUT_BUFFER_SIZE 300000

G_DEFINE_TYPE (GooTiVideoEncoder, goo_ti_video_encoder, GOO_TYPE_COMPONENT)

GType
goo_ti_video_encoder_control_rate_get_type ()
{
	static GType goo_ti_video_encoder_control_rate_type = 0;
	static GEnumValue goo_ti_video_encoder_control_rate[] =
	{
		{ GOO_TI_VIDEO_ENCODER_CR_DISABLE, "0", "Disable" },
		{ GOO_TI_VIDEO_ENCODER_CR_VARIABLE, "1", "Variable" },
		{ GOO_TI_VIDEO_ENCODER_CR_CONSTANT, "2", "Constant" },
		{ GOO_TI_VIDEO_ENCODER_CR_VARIABLE_SKIP, "3", "Variable Skip" },
		{ GOO_TI_VIDEO_ENCODER_CR_CONSTANT_SKIP, "4", "Constant Skip" },
		{ 0, NULL, NULL },
	};

	if (!goo_ti_video_encoder_control_rate_type)
	{
		goo_ti_video_encoder_control_rate_type = g_enum_register_static
			("GooTiVideoEncoderControlRate",
			 goo_ti_video_encoder_control_rate);
	}

	return goo_ti_video_encoder_control_rate_type;
}

static void
goo_ti_video_encoder_load_parameters (GooComponent* component)
{
	g_assert (GOO_IS_TI_VIDEO_ENCODER (component));
	GooTiVideoEncoder* self = GOO_TI_VIDEO_ENCODER (component);
...
static gboolean
_goo_ti_video_encoder_set_frame_interval (GooTiVideoEncoder* self, guint frame_interval)
{
	g_assert (self != NULL);
...
static void
goo_ti_video_encoder_validate_ports_definition (GooComponent* component)
{
	g_assert (GOO_IS_TI_VIDEO_ENCODER (component));
	GooTiVideoEncoder* self = GOO_TI_VIDEO_ENCODER (component);
...
static void
goo_ti_video_encoder_set_parameters (GooComponent* component)
{
	g_assert (GOO_IS_TI_VIDEO_ENCODER (component));
	GooTiVideoEncoder* self = GOO_TI_VIDEO_ENCODER (component);
...
static void
goo_ti_video_encoder_init (GooTiVideoEncoder* self)
{
...
static void
goo_ti_video_encoder_set_property (GObject* object, guint prop_id,
				   const GValue* value, GParamSpec* spec)
{
        g_assert (GOO_IS_TI_VIDEO_ENCODER (object));
        GooTiVideoEncoder* self = GOO_TI_VIDEO_ENCODER (object);
	...
	GOO_OBJECT_DEBUG (self, "Entering");
		...
		_goo_ti_video_encoder_set_frame_interval (self, g_value_get_uint (value));
		break;
        default:
...
static void
goo_ti_video_encoder_get_property (GObject* object, guint prop_id,
...
static void
goo_ti_video_encoder_finalize (GObject* object)
{
	g_assert (GOO_IS_TI_VIDEO_ENCODER (object));
	GooTiVideoEncoder* self = GOO_TI_VIDEO_ENCODER (object);
	...
	(*G_OBJECT_CLASS (goo_ti_video_encoder_parent_class)->finalize) (object);

	return;
}

static void
goo_ti_video_encoder_class_init (GooTiVideoEncoderClass* klass)
{

	GObjectClass* g_klass = G_OBJECT_CLASS (klass);

        g_klass->set_property = goo_ti_video_encoder_set_property;
        g_klass->get_property = goo_ti_video_encoder_get_property;
	g_klass->finalize = goo_ti_video_encoder_finalize;
	...
	GooComponentClass* o_klass = GOO_COMPONENT_CLASS (klass);
        o_klass->set_parameters_func = goo_ti_video_encoder_set_parameters;
	o_klass->load_parameters_func = goo_ti_video_encoder_load_parameters;
	o_klass->validate_ports_definition_func =
		goo_ti_video_encoder_validate_ports_definition;
	/* o_klass->set_state_idle_func = goo_ti_video_encoder_set_state_idle; */
	return;
}
 ...
 **/
void
goo_ti_video_encoder_set_control_rate (GooTiVideoEncoder* self,
 ...
 *
 * Return value: GooTiVideoEncoderControlRate enum value.
 **/
GooTiVideoEncoderControlRate
goo_ti_video_encoder_get_control_rate (GooTiVideoEncoder* self)
{
        g_assert (self != NULL);

        GooTiVideoEncoderControlRate retval;
