...
#define ID "OMX.TI.AAC.decode"
#define DASF_PARAM_NAME "OMX.TI.index.config.aacdecHeaderInfo"
#define DATAPATH_PARAM_NAME "OMX.TI.index.config.aacdec.datapath"
#define FRAME_PARAM_NAME "OMX.TI.index.config.aacdecHeaderInfo"
#define STREAMID_PARAM_NAME "OMX.TI.index.config.aacidecstreamIDinfo"
#define INPUT_BUFFERSIZE 2000
#define OUTPUT_BUFFERSIZE 8192

G_DEFINE_TYPE (GooTiAacDec, goo_ti_aacdec, GOO_TYPE_TI_AUDIO_DECODER)

static void
goo_ti_aacdec_init (GooTiAacDec* self)
{
...
static void
goo_ti_aacdec_finalize (GObject* object)
{
        g_assert (GOO_IS_TI_AACDEC (object));
        GooTiAacDec* self = GOO_TI_AACDEC (object);
        ...
        (*G_OBJECT_CLASS (goo_ti_aacdec_parent_class)->finalize) (object);

        return;
}

static void
goo_ti_aacdec_load_parameters (GooComponent* component)
{
	g_assert (GOO_IS_TI_AACDEC (component));
	GooTiAacDec* self = GOO_TI_AACDEC (component);
	...
	GOO_OBJECT_DEBUG (self, "");
        ...
        /* self->param = g_new0 (OMX_AUDIO_PARAM_MP3TYPE, 1); */
...
static void
goo_ti_aacdec_set_parameters (GooComponent* component)
{
        g_assert (GOO_IS_TI_AACDEC (component));
        GooTiAacDec* self = GOO_TI_AACDEC (component);
        g_assert (self->input_port_param != NULL);
...
static void
goo_ti_aacdec_validate_ports_definitions (GooComponent* component)
{
	g_assert (GOO_IS_TI_AACDEC (component));
	GooTiAacDec* self = GOO_TI_AACDEC (component);
	g_assert (self->input_port_param != NULL);
...
static void
goo_ti_aacdec_class_init (GooTiAacDecClass* klass)
{
	GObjectClass* g_klass = G_OBJECT_CLASS (klass);
	g_klass->finalize = goo_ti_aacdec_finalize;

	GooComponentClass* o_klass = GOO_COMPONENT_CLASS (klass);
	o_klass->load_parameters_func = goo_ti_aacdec_load_parameters;
	o_klass->set_parameters_func = goo_ti_aacdec_set_parameters;
	o_klass->validate_ports_definition_func =
		goo_ti_aacdec_validate_ports_definitions;
