...
G_DEFINE_TYPE (GooTiGsmHrEnc, goo_ti_gsmhrenc, GOO_TYPE_TI_AUDIO_ENCODER)

static void
goo_ti_gsmhrenc_init (GooTiGsmHrEnc* self)
{
...
static void
goo_ti_gsmhrenc_finalize (GObject* object)
{
        g_assert (GOO_IS_TI_GSMHRENC (object));
        GooTiGsmHrEnc* self = GOO_TI_GSMHRENC (object);
	...
	if (G_LIKELY (self->input_port_param))
	{
        ...
        (*G_OBJECT_CLASS (goo_ti_gsmhrenc_parent_class)->finalize) (object);

        return;
}

static void
goo_ti_gsmhrenc_load_parameters (GooComponent* component)
{
	g_assert (GOO_IS_TI_GSMHRENC (component));
	GooTiGsmHrEnc* self = GOO_TI_GSMHRENC (component);
...

	{
	    ...
	    self->output_port_param = g_new0 (OMX_AUDIO_PARAM_GSMHRTYPE, 1);
	    GOO_INIT_PARAM (self->output_port_param, OMX_AUDIO_PARAM_GSMHRTYPE);
/*	    goo_component_get_parameter_by_index (component,
...
static void
goo_ti_gsmhrenc_set_parameters (GooComponent* component)
{
	g_assert (GOO_IS_TI_GSMHRENC (component));
	GooTiGsmHrEnc* self = GOO_TI_GSMHRENC (component);
...
static void
goo_ti_gsmhrenc_validate_ports (GooComponent* component)
{
	g_assert (GOO_IS_TI_GSMHRENC (component));
	GooTiGsmHrEnc* self = GOO_TI_GSMHRENC (component);
	g_assert (self->input_port_param != NULL);
	...
	/* input */
	{
		...
		def->nBufferSize = GOO_TI_GSMHRENC_INPUT_BUFFER_SIZE;
	...
	/* output */
	{
		...
		def->nBufferSize = GOO_TI_GSMHRENC_OUTPUT_BUFFER_SIZE;
...
static void
goo_ti_gsmhrenc_class_init (GooTiGsmHrEncClass* klass)
{

	GObjectClass* g_klass = G_OBJECT_CLASS (klass);
	g_klass->finalize = goo_ti_gsmhrenc_finalize;

	GooComponentClass* o_klass = GOO_COMPONENT_CLASS (klass);
	o_klass->load_parameters_func = goo_ti_gsmhrenc_load_parameters;
	o_klass->set_parameters_func = goo_ti_gsmhrenc_set_parameters;
	o_klass->validate_ports_definition_func =
		goo_ti_gsmhrenc_validate_ports;
